...
class SFHIR(PPM.Service):
    ...
    def _find_resources(obj, resource_type=None):
        ...
        if isinstance(obj, Resource):
            ...
            if type(obj) is Bundle:
                ...
                # Get entries
                return SFHIR._find_resources(obj.entry, resource_type=resource_type)

            else:

                # Object is resource, return it
                return [obj.as_json()] if obj.resource_type == resource_type else []

        # Check if is a search bundle entry
        if type(obj) is BundleEntry:

            # Return its resource
            return SFHIR._find_resources(obj.resource, resource_type=resource_type)
        ...
        if type(obj) is list:
            ...
            resources = []
            for r in obj:
                resources.extend(SFHIR._find_resources(r, resource_type=resource_type))
        ...
        if type(obj) is dict:
            ...
            # Check for bundle entry
                ...
                return [obj] if obj['resourceType'] == resource_type else []
    ...
    def _find_resource(obj, resource_type=None):
        ...
        :return: object
        """
        return next(iter(SFHIR._find_resources(obj, resource_type)), None)
    ...
    def is_ppm_research_subject(research_subject):
        ...
        """
        if research_subject.get('identifier', {}).get('system') == SFHIR.research_subject_identifier_system and \
    ...
    def is_ppm_research_study(research_study):
        ...
        for identifier in research_study.get('identifier', []):
            if identifier.get('system') == SFHIR.research_study_identifier_system and \
    ...
    def _patient_query(identifier):
        ...
        # Check for an email address
            ...
            # An email address
            return {'identifier': '{}|{}'.format(SFHIR.patient_email_identifier_system, identifier)}
    ...
    @staticmethod
    def _patient_id(request, identifier):
        """
        ...
        # Check for an email address
            ...
            # An email address
            return SFHIR.get_ppm_id(request, identifier)
        ...
        # Check for a bundle
                ...
                SFHIR._find_resource(identifier, 'Patient'):

            return SFHIR._find_resource(identifier, 'Patient')['resource']['id']

        # Check for a bundle
        elif type(identifier) is Bundle and SFHIR._find_resource(identifier.as_json(), 'Patient'):

            return SFHIR._find_resource(identifier.as_json(), 'Patient')['resource']['id']
    ...
    def _patient_resource_query(identifier, key='patient'):
        ...
        # Check for an email address
            ...
            # An email address
            return {'{}:patient.identifier'.format(key): '{}|{}'.format(SFHIR.patient_email_identifier_system, identifier)}
    ...
    def create_ppm_research_study(request, project, title):
        ...
        """
        research_study_data = SFHIR.Resources.ppm_research_study(project, title)

        # Use the FHIR client lib to validate our resource.
        # "If-None-Exist" can be used for conditional create operations in SFHIR.
        ...
        return SFHIR.post(request=request, path='/', data=bundle.as_json())
    ...
    def create_ppm_research_subject(request, project, patient_id):
        ...
        study = SFHIR._query_resource(request, 'ResearchStudy', query={
            'identifier': '{}|{}'.format(SFHIR.research_study_identifier_system, project)
        })
        if not study:
            SFHIR.create_ppm_research_study(request, project, PPM.Project.title(project))

        # Generate resource data
        research_subject_data = SFHIR.Resources.ppm_research_subject(project, 'Patient/{}'.format(patient_id))
    ...
    def create_ppm_device(request, patient_id, item, identifier=None, shipped=None, returned=None):
        ...
        Creates a project list if not already created
        """
        device_data = SFHIR.Resources.ppm_device(
            item=item,
    ...
    def create_patient(request, form, project):
        ...
        patient_data = SFHIR.Resources.patient(form)
        ...
        patient_data.get('identifier', []).append({
            'system': SFHIR.patient_identifier_system,
        ...
        flag = Flag(SFHIR.Resources.enrollment_flag(patient_uuid.urn, 'registered'))
        ...
        research_subject_data = SFHIR.Resources.ppm_research_subject(project, patient_uuid.urn, 'candidate')
    ...
    def create_patient_enrollment(request, patient_id, status='registered'):
        ...
        # Use the FHIR client lib to validate our resource.
        flag = Flag(SFHIR.Resources.enrollment_flag('Patient/{}'.format(patient_id), status))
        ...
        return SFHIR.post(request=request, path='/Flag', data=flag.as_json())
    ...
    def create_communication(request, patient_id, content, identifier):
        ...
        # Use the FHIR client lib to validate our resource.
        communication = Communication(SFHIR.Resources.communication(
            patient_ref='Patient/{}'.format(patient_id),
            identifier=identifier,
            content=content,
        ...
        return SFHIR.post(request=request, path='/Communication', data=communication)
    ...
    def create_point_of_care_list(request, patient_id, point_of_care_list):
        ...
        coding = Coding()
        coding.system = SFHIR.SNOMED_VERSION_URI
        coding.code = SFHIR.SNOMED_LOCATION_CODE
        ...
        bundle_item_list_request.ifNoneExist = str(
            Query({
                'patient': patient_id,
                'code': SFHIR.SNOMED_VERSION_URI + "|" + SFHIR.SNOMED_LOCATION_CODE,
                'status': 'current'
            })
        )
        ...
        return SFHIR.post(request=request, path='/', data=full_bundle.as_json())
    ...
    def _query_resources(request, resource_type, query=None, bundled=False):
        ...
        while query is not None:
            ...
            # Parse the JSON.
            bundle = SFHIR.get(request, path=f'/{resource_type}', data=query)
    ...
    def _query_resource(request, resource_type, query=None):
        ...
        # Query
        resources = SFHIR._query_resources(request, resource_type, query)
        if not resources:
            return None
    ...
    def _query_bundle(request, resource_type, query=None):
        ...
        if next_url:
            ...
            bundle['entry'].extend(SFHIR._query_resources(request, resource_type, query, bundled=True))
    ...
    def _get_resource(request, resource_type, _id):
        ...
        return SFHIR.get(request, path=f'/{resource_type}/{_id}')
    ...
    def query_participants(request, studies=None, enrollments=None, active=None, testing=False):
        ...
        # Peel out patients
        bundle = SFHIR._query_bundle(request, 'Patient', query)
        ...
        patient_studies = {entry.resource.individual.reference.split('/')[1]:
                           {'study': SFHIR.get_study_from_research_subject(entry.resource),
                           ...
                           SFHIR.is_ppm_research_subject(entry.resource.as_json())}
        ...
        for patient in [entry.resource for entry in bundle.entry if entry.resource.resource_type == 'Patient']:
            try:
                ...
                email = next(identifier.value for identifier in patient.identifier
                             if identifier.system == SFHIR.patient_email_identifier_system)
                ...
                date_registered = SFHIR._format_date(patient_study.get('date_registered'), '%m/%d/%Y')
                ...
                if patient_enrollment.get('date_accepted'):
                    patient_dict['date_accepted'] = SFHIR._format_date(patient_enrollment['date_accepted'], '%m/%d/%Y')

                # Wrap the patient resource in a fake bundle and flatten them
                flattened_patient = SFHIR.flatten_patient({'entry': [{'resource': patient.as_json()}]})
    ...
    def query_patients(request, study=None, enrollment=None, active=None, testing=False, include_deceased=True):
        ...
        return SFHIR.query_participants(request, studies, enrollments, active, testing)

    @staticmethod
    def get_participant(request, patient, flatten_return=False):

        # Add query for patient
        data = SFHIR._patient_query(patient)
        data['_include'] = '*'
        data['_revinclude'] = '*'

        # Make the FHIR request.
        bundle = SFHIR.get(request, path='Patient', data=data)
        if not bundle.get('entry'):
            return {}

        # Return accordingly
        if flatten_return:
            return SFHIR.flatten_participant(request, bundle)
        else:
    ...
    def get_patient(request, patient, flatten_return=False):
        ...
        if bundle:
            ...
            if flatten_return:
                return SFHIR.flatten_patient(bundle)
            else:
    ...
    @staticmethod
    def get_composition(request, patient=None, flatten_return=False):
        ...
        if patient:
            query.update(SFHIR._patient_query(patient))

        # Fetch it
        bundle = SFHIR.get(request, path='Composition', query=query)
        if flatten_return:
            return SFHIR.flatten_consent_composition(bundle)
        else:
            return bundle
    ...
    def query_patient_id(request, email):
        ...
        # Get the patient
        return SFHIR.get_patient(request, email)['id']
    ...
    def query_ppm_devices(request, patient=None, item=None, identifier=None, flatten_return=False):
        ...
        if item:
            query = {
                'type': '{}|{}'.format(SFHIR.device_coding_system, item),
            }
        else:

            query = {
                'type': '{}|'.format(SFHIR.device_coding_system),
            }

        # Check for an identifier
        if identifier:
            query['identifier'] = '{}|{}'.format(SFHIR.device_identifier_system, identifier)

        # Update for the patient query
        if patient:
            query.update(SFHIR._patient_resource_query(patient))

        # Make the fetch
        bundle = SFHIR.get(request, path='/Device', data=query)
        if flatten_return:
            return [SFHIR.flatten_ppm_device(resource['resource']) for
                    resource in bundle.get('entry', [])]
        else:
            return [entry['resource'] for entry in bundle.get('entry', [])]
    ...
    def query_ppm_research_subjects(request, patient=None, flatten_return=False):
        ...
        query = {
            'identifier': '{}|'.format(SFHIR.research_subject_identifier_system),
        }
        ...
        # Make the fetch
        bundle = SFHIR.get(request, path='/ResearchSubject', data=query)
        if flatten_return:
            return [SFHIR.flatten_research_subject(resource['resource']) for
                    resource in bundle.get('entry', [])]
        else:
            return [entry['resource'] for entry in bundle.get('entry', [])]
    ...
    def query_research_subjects(request, patient=None, flatten_return=False):
        ...
        if patient:
            # Get flags for current user
            query = SFHIR._patient_resource_query(patient)
        else:
            query = {}
        ...
        research_subjects = [entry['resource'] for entry in bundle.get('entry', [])
                             if entry['resource'].get('study', {}).get('reference', None) not in
                             ['ResearchStudy/ppm-{}'.format(study.value) for study in PPM.Project]]

        if flatten_return:
            return [SFHIR.flatten_research_subject(resource)
                    for resource in research_subjects]
        else:
            return research_subjects

    @staticmethod
    def query_enrollment_flag(request, patient=None, flatten_return=False):

        # Get flags for current user
        if patient:
            query = SFHIR._patient_resource_query(patient, 'subject')
        else:
            query = {}

        # Make the fetch
        bundle = SFHIR.get(request, path='/Flag', data=query)
        if flatten_return:
            return SFHIR.flatten_enrollment_flag(bundle)
        else:
            return bundle
    ...
    def get_questionnaire_response(request, patient=None, questionnaire_id=None, flatten_return=False):
        ...
        query = {}
        if patient:
            query.update(SFHIR._patient_resource_query(patient, 'source'))

        if questionnaire_id:
            query['questionnaire'] = f'Questionnaire/{questionnaire_id}'

        # Make the fetch
        bundle = SFHIR.get(request, path='/QuestionnaireResponse', data=query)
        if flatten_return:
            return SFHIR.flatten_questionnaire_response(bundle, questionnaire_id)
        else:
            return next((entry['resource'] for entry in bundle.get('entry', [])), None)

    @staticmethod
    def query_document_references(request, patient=None, query=None):
        """
        ...
        if patient:
            _query.update(SFHIR._patient_resource_query(patient))

        if query:
            _query.update(query)

        # TODO: Remove the Bundled flag to not manage BundleEntry resources
        return SFHIR._query_resources(request, 'DocumentReference', query=_query, bundled=True)

    @staticmethod
    def query_data_document_references(request, patient=None, provider=None, status=None):
        """
        Queries the current user's FHIR record for any DocumentReferences related to this type
        :return: A list of DocumentReference resources
        :rtype: list
        ...
        if provider:
            query['type'] = f'{SFHIR.data_document_reference_identifier_system}|{provider}'
        else:
            query['type'] = f'{SFHIR.data_document_reference_identifier_system}|'
        ...
        # TODO: Remove the Bundled flag to not manage BundleEntry resources
        return SFHIR._query_resources(request, 'DocumentReference', query=query, bundled=True)
    ...
    def query_enrollment_status(request, patient_ref):
        ...
        try:
            # Make the FHIR request.
            response = SFHIR.query_enrollment_flag(request, patient_ref)
    ...
    def query_ppm_research_studies(request, email, flatten_return=True):
        ...
        research_subjects = SFHIR.query_ppm_research_subjects(request, email, flatten_return=False)
        ...
        # Fetch them
        bundle = SFHIR.get(request, path='/ResearchStudy', data={'_id': ','.join(research_study_ids)})

        # Get the IDs
        research_studies = bundle.get('entry', [])
    ...
    def query_research_studies(request, email, flatten_return=True):
        ...
        # Find Research subjects (without identifiers, so as to exclude PPM resources)
        research_subjects = SFHIR.query_research_subjects(request, email, flatten_return=False)
    ...
    @staticmethod
    def get_point_of_care_list(request, patient=None, flatten_return=False):
        """
        ...
        bundle = SFHIR._query_bundle(request, 'List', query=query)

        if flatten_return:
            return SFHIR.flatten_list(bundle, 'Organization')
        else:
            return next((entry['resource'] for entry in bundle.as_json().get('entry', [])), None)

    @staticmethod
    def query_ppm_communications(request, patient=None, identifier=None):
        """
        ...
        query = {}
        if patient:
            query.update(SFHIR._patient_resource_query(patient, 'recipient'))

        if identifier:
            query['identifier'] = f'{SFHIR.ppm_comm_identifier_system}|{identifier}'

        # Find all resources
        # TODO: Remove the Bundled flag to not manage BundleEntry resources
        resources = SFHIR._query_resources(request, 'Communication', query=query, bundled=True)
    ...
    def update_patient(request, fhir_id, form):
        ...
        # Get their resource
        patient = SFHIR.get(request, path=f'/Patient/{fhir_id}')
        ...
        if phone:
            for telecom in patient.get('telecom', []):
                if telecom['system'] == SFHIR.patient_phone_telecom_system:
                    telecom['value'] = phone
                    break
            else:
                # Add it
                patient.setdefault('telecom', []).append(
                    {'system': SFHIR.patient_phone_telecom_system, 'value': phone}
                )

        email = form.get('contact_email')
        if email:
            for telecom in patient.get('telecom', []):
                if telecom['system'] == SFHIR.patient_email_telecom_system:
                    telecom['value'] = email
                    break
            else:
                # Add it
                patient.setdefault('telecom', []).append(
                    {'system': SFHIR.patient_email_telecom_system, 'value': email}
                )
        ...
        if referral:
            for extension in patient.get('extension', []):
                if extension['url'] == SFHIR.referral_extension_url:
                    extension['valueString'] = referral
                    break
            else:
                # Add it
                patient.setdefault('extension', []).append(
                    {'url': SFHIR.referral_extension_url, 'valueString': referral}
                )
        ...
        # Put it
        return SFHIR.put(request, path=f'/Patient/{fhir_id}')
    ...
    def update_patient_active(request, patient_id, active):
        ...
        # Get patient ID
        ppm_id = SFHIR._patient_id(request, patient_id)

        return SFHIR.request('patch', headers=headers, request=request,
                             path=f'/Patient/{ppm_id}',
                             data=patch, check=True)

    @staticmethod
    def update_ppm_device(request, patient_id, item, identifier=None, shipped=None, returned=None):

        # Get the device
        device = next(iter(SFHIR.query_ppm_devices(request, patient=patient_id, item=item)), None)
        if not device:
        ...
        if identifier:
            ...
            ppm_identifier = next((_id for _id in identifiers
                                   if _id.get('system') == SFHIR.device_identifier_system), None)
            if ppm_identifier:
                ...
                # Add a new one
                identifiers.append({'system': SFHIR.device_identifier_system, 'value': identifier.lower()})
        ...
        return SFHIR.put(request, path=f'/Device/{device["id"]}', data=device)
    ...
    def update_patient_enrollment(request, patient_id, status):
        ...
        # Build the query
        query = SFHIR._patient_resource_query(patient_id, key='subject')

        # Fetch the flag.
        resource = SFHIR._query_resource(request, 'Flag', query)
        ...
        if not resource:
            ...
            # Create it.
            return SFHIR.create_patient_enrollment(request, patient_id, status)
            ...
            # Post the transaction
            return SFHIR.put(request, path=f'/Flag/{flag.id}', data=flag.as_json())
    ...
    def update_ppm_research_subject(request, patient_id, study=None, start=None, end=None):
        ...
        # Build the query
        query = SFHIR._patient_resource_query(patient_id)

        # Build study identifier
        study_query = '{}|'.format(SFHIR.research_subject_identifier_system)
        if study:
        ...
        research_subjects = SFHIR._query_resources(request, 'ResearchSubject', query=query)
        ...
        for research_subject_id in [resource['id'] for resource in research_subjects]:
            ...
            # Do the update
            SFHIR.update_research_subject(request, patient_id, research_subject_id, start, end)
    ...
    def update_point_of_care_list(request, patient, point_of_care):
        ...
        point_of_care_list = SFHIR.get_point_of_care_list(request, patient, flatten_return=False)
        points_of_care = SFHIR.flatten_list(point_of_care_list)
        ...
        org = SFHIR._query_resource(request, 'Organization', query={'name': point_of_care})
        ...
        # Post the transaction
        SFHIR.post(request, path='/', data=bundle.as_json())
    ...
    def update_twitter(request, patient_id, handle=None, uses_twitter=None):
        ...
        # Fetch the Patient.
        patient = SFHIR.get_patient(request, patient_id)

        # Check if handle submitted or not
        if handle:

            # Set the value
            twitter = {'system': SFHIR.patient_twitter_telecom_system, 'value': 'https://twitter.com/' + handle}
        ...
        if handle is not None or uses_twitter is not None:
            ...
            if not extension:
                ...
                extension = {
                    'url': SFHIR.twitter_extension_url,
                    'valueBoolean': value
                }
        ...
        return SFHIR.request('put', request=request,
                             path=f'/Patient/{patient["id"]}',
                             data=patient, check=True)
    ...
    def update_patient_extension(request, patient_id, extension_url, value=None):
        ...
        return SFHIR.put(request, path=f'/Patient/{patient["id"]}', data=patient)
    ...
    def update_picnichealth(request, patient_id, registered=True):
        ...
        # Check for an existing Twitter status extension
        extension = next((extension for extension in patient.get('extension', [])
                          if SFHIR.picnichealth_extension_url in extension.get('url')), None)
        if extension:
            ...
            # Add an extension indicating their use of Twitter
            extension = {
                    'url': SFHIR.picnichealth_extension_url,
    ...
    def _delete_resources(request, source_resource_type, source_resource_id, target_resource_types=[]):
        ...
        try:
            ...
            # Make the request.
            source_response = SFHIR.get(request, path='/', data=query)
            ...
            # Do the delete.
            deleted = SFHIR.post(request, path='/', data=transaction)
            if deleted:
    ...
    def _delete_resource(request, resource_type, resource_id):
        ...
        # Do the delete.
        return SFHIR.delete(request, path=f'/{resource_type}/{resource_id}')
    ...
    def delete_participant(patient_id):
        ...
        # Do the delete
        SFHIR._delete_resources('Patient', patient_id, resources)
    ...
    def delete_patient(request, patient_id):
        ...
        return SFHIR.delete(request, path=f'/Patient/{SFHIR._patient_id(request, patient_id)}')
    ...
    def delete_research_subjects(request, patient_id):
        ...
        """
        # Find it
        research_subjects = SFHIR.query_research_subjects(request, patient_id, flatten_return=False)
        for research_subject in research_subjects:

            # Attempt to delete the patient and all related resources.
            SFHIR.delete(request, path=f'/ResearchSubject/{research_subject["id"]}')
    ...
    def delete_point_of_care_list(request, patient_id):
        ...
        """
        # Find it
        point_of_care_list = SFHIR.get_point_of_care_list(request, patient_id, flatten_return=False)
        if point_of_care_list:

            # Attempt to delete the patient and all related resources.
            SFHIR.delete(request, path=f'/List/{point_of_care_list["id"]}')
    ...
    def delete_questionnaire_response(request, patient_id, project):
        ...
        questionnaire_id = SFHIR.questionnaire_id(project)

        # Find it
        questionnaire_response = SFHIR.get_questionnaire_response(request, patient_id, questionnaire_id)
        if questionnaire_response:

            # Delete it
            SFHIR.delete(request, path=f'/QuestionnaireResponse/{questionnaire_response["id"]}')
    ...
    def delete_consent(request, patient_id, project):
        ...
        # Make the FHIR request.
        return SFHIR.post(request, path='/', data=json.dumps(transaction))
    ...
    def get_ppm_research_studies(request, bundle, flatten_result=True):
        ...
        # Find Research subjects (without identifiers, so as to exclude PPM resources)
        subjects = SFHIR.get_ppm_research_subjects(bundle, flatten_result=False)
        if not subjects:
            logger.debug('No Research Subjects, no Research Studies')
            return None
        ...
        # Make the query
        query = {
            '_id': ','.join(research_study_ids)
        }

        # Get the IDs
        research_studies = SFHIR._query_resources(request, resource_type='ResearchStudy', query=query)
    ...
    def get_research_studies(request, bundle, flatten_result=True):
        ...
        # Find Research subjects (without identifiers, so as to exclude PPM resources)
        subjects = SFHIR.get_research_subjects(bundle, flatten_result=False)
        if not subjects:
            logger.debug('No Research Subjects, no Research Studies')
            return None
    ...
    def get_ppm_research_subjects(bundle, flatten_result=True):
        ...
        if flatten_result:
            # Return the titles
            return [SFHIR.flatten_research_subject(resource) for resource in research_subjects]
        else:
    ...
    def get_name(patient, full=False):
        ...
        if not names:
            ...
            email = next((identifier['value'] for identifier in patient['identifier'] if
                          identifier.get('system') == SFHIR.patient_email_identifier_system), None)
    ...
    def flatten_participant(request, bundle):
        ...
        try:
            ...
            participant = SFHIR.flatten_patient(bundle)
            if not participant:
            ...
            studies = SFHIR.flatten_ppm_studies(bundle)
            if len(studies) != 1:
            ...
            participant['date_registered'] = SFHIR._format_date(studies[0]['start'], '%m/%d/%Y')

            # Get the enrollment properties
            enrollment = SFHIR.flatten_enrollment(bundle)
            ...
            if enrollment.get('start'):
                ...
                # Convert time zone to assumed ET
                participant['enrollment_accepted_date'] = SFHIR._format_date(enrollment['start'], '%m/%d/%Y')
            ...
            if enrollment.get('end'):
                ...
                # Convert time zone to assumed ET
                participant['enrollment_terminated_date'] = SFHIR._format_date(enrollment['end'], '%m/%d/%Y')
            #
            # else:
            #     participant['enrollment_terminated_date'] = ''

            # Flatten consent composition
            participant['composition'] = SFHIR.flatten_consent_composition(bundle)

            # Get the project
            _questionnaire_id = SFHIR.questionnaire_id(participant['project'])

            # Parse out the responses
            participant['questionnaire'] = SFHIR.flatten_questionnaire_response(bundle, _questionnaire_id)

            # Flatten points of care
            participant['points_of_care'] = SFHIR.flatten_list(bundle, 'Organization')

            # Flatten consent composition
            participant['devices'] = SFHIR.flatten_ppm_devices(bundle)

            # Check for research studies
            research_studies = SFHIR.get_research_studies(request, bundle)
            if research_studies:
            ...
            if participant['project'] == PPM.Study.ASD.value:
                ...
                # Get the questionnaire ID
                quiz_id = SFHIR.consent_questionnaire_id(participant)

                # Sort it.
                participant['consent_quiz'] = SFHIR.flatten_questionnaire_response(bundle, quiz_id)
    ...
    def flatten_questionnaire_response(bundle_dict, questionnaire_id):
        ...
        questions = SFHIR._questions(questionnaire.item)
        answers = SFHIR._answers(questionnaire_response.item)
        ...
        formatted_authored_date = SFHIR._format_date(authored_date, '%m/%d/%Y')

        return {
            'ppm_id': SFHIR._get_referenced_id(questionnaire_response.as_json(), 'Patient'),
    ...
    def _questions(items):
        ...
        for item in items:
            ...
            if item.type == 'display':
                ...
                # Get answers
                sub_questions = SFHIR._questions(item.item)
    ...
    def _answers(items):
        ...
        for item in items:
            ...
            if item.item:
                # Get answers
                sub_answers = SFHIR._answers(item.item)
    ...
    def flatten_patient(bundle_dict):
        ...
        patient['email'] = next((identifier['value'] for identifier in resource.get('identifier', [])
                                 if identifier.get('system') == SFHIR.patient_email_identifier_system))
        if not patient.get('email'):
            logger.error('Could not parse email from Patient/{}! This should not be possible'.format(resource['id']))
            return {}

        # Get status
        patient['active'] = SFHIR._get_or(resource, ['active'], '')

        # Get the remaining optional properties
        patient["firstname"] = SFHIR._get_or(resource, ['name', 0, 'given', 0], '')
        patient["lastname"] = SFHIR._get_or(resource, ['name', 0, 'family'], '')
        patient["street_address1"] = SFHIR._get_or(resource, ['address', 0, 'line', 0], '')
        patient["street_address2"] = SFHIR._get_or(resource, ['address', 0, 'line', 1], '')
        patient["city"] = SFHIR._get_or(resource, ['address', 0, 'city'], '')
        patient["state"] = SFHIR._get_or(resource, ['address', 0, 'state'], '')
        patient["zip"] = SFHIR._get_or(resource, ['address', 0, 'postalCode'], '')
        patient["phone"] = SFHIR._get_or(resource, ['telecom', 0, 'postalCode'], '')

        # Check for deceased
        if SFHIR._get_or(resource, ['deceasedDateTime'], None):
            patient['deceased'] = SFHIR._format_date(resource['deceasedDateTime'], '%m/%d/%Y')

        # Parse telecom properties
        patient['phone'] = next((telecom.get('value', '') for telecom in resource.get('telecom', [])
                                 if telecom.get('system') == SFHIR.patient_phone_telecom_system), '')
        patient['twitter_handle'] = next((telecom.get('value', '') for telecom in resource.get('telecom', [])
                                         if telecom.get('system') == SFHIR.patient_twitter_telecom_system), '')
        patient['contact_email'] = next((telecom.get('value', '') for telecom in resource.get('telecom', [])
                                        if telecom.get('system') == SFHIR.patient_email_telecom_system), '')
        ...
        patient['picnichealth'] = next((extension['valueBoolean'] for extension in resource.get('extension', [])
                                        if SFHIR.picnichealth_extension_url in extension.get('url')), False)
    ...
    def flatten_research_subject(resource):
        ...
        record['start'] = SFHIR._get_or(resource, ['period', 'start'])
        record['end'] = SFHIR._get_or(resource, ['period', 'end'])

        # Get the study ID
        record['study'] = SFHIR.get_study_from_research_subject(resource)

        # Link back to participant
        record['ppm_id'] = SFHIR._get_referenced_id(resource, 'Patient')
    ...
    def flatten_research_study(resource):
        ...
        record['start'] = SFHIR._get_or(resource, ['period', 'start'])
        record['end'] = SFHIR._get_or(resource, ['period', 'end'])
        record['status'] = SFHIR._get_or(resource, ['status'])
        record['title'] = SFHIR._get_or(resource, ['title'])

        if resource.get('identifier'):
            record['identifier'] = SFHIR._get_or(resource, ['identifier', 0, 'value'])
    ...
    def flatten_ppm_studies(bundle):
        ...
        # Collect them
        research_subjects = []
        for research_subject in SFHIR._find_resources(bundle, 'ResearchSubject'):

            # Ensure it's the PPM kind
            if SFHIR.is_ppm_research_subject(research_subject):

                # Flatten it and add it
                research_subjects.append(SFHIR.flatten_research_subject(research_subject))
    ...
    def flatten_ppm_devices(bundle):
        ...
        for device in SFHIR._find_resources(bundle, 'Device'):

            # Ensure it's a PPM device
            for identifier in device.get('identifier', []):
                if identifier.get('system') == SFHIR.device_identifier_system:

                    # Flatten it
                    devices.append(SFHIR.flatten_ppm_device(device))
    ...
    def flatten_ppm_device(resource):
        ...
        # Try and get the values
        record['status'] = SFHIR._get_or(resource, ['status'])
        record['shipped'] = SFHIR._get_or(resource, ['manufactureDate'])
        record['returned'] = SFHIR._get_or(resource, ['expirationDate'])
        ...
        for coding in SFHIR._get_or(resource, ['type', 'coding'], []):
            if coding.get('system') == SFHIR.device_coding_system:
    ...
    def flatten_enrollment(bundle):
        ...
        """
        for flag in SFHIR._find_resources(bundle, 'Flag'):

            # Ensure it's the enrollment flag
            if SFHIR.enrollment_flag_coding_system == SFHIR._get_or(flag, ['code', 'coding', 0, 'system']):

                # Flatten and return it
                return SFHIR.flatten_enrollment_flag(flag)

            logger.error('No Flag with coding: {} found'.format(SFHIR.enrollment_flag_coding_system))
    ...
    def flatten_enrollment_flag(resource):
        ...
        # Try and get the values
        record['enrollment'] = SFHIR._get_or(resource, ['code', 'coding', 0, 'code'])
        record['status'] = SFHIR._get_or(resource, ['status'])
        record['start'] = SFHIR._get_or(resource, ['period', 'start'])
        record['end'] = SFHIR._get_or(resource, ['period', 'end'])
    ...
    def flatten_consent_composition(bundle_json):
        ...
        # Add link IDs.
        SFHIR._fix_bundle_json(bundle_json)
        ...
        if incoming_bundle.total > 0:
            ...
            for bundle_entry in incoming_bundle.entry:
                if bundle_entry.resource.resource_type == "Consent":
                    ...
                    # Format it
                    consent_object["date_signed"] = SFHIR._format_date(date_time, '%m/%d/%Y')
                    ...
                    if signed_consent.except_fhir:
                        for consent_exception in signed_consent.except_fhir:
                            ...
                            consent_exceptions.append(SFHIR._exception_description(display))
                        ...
                        if questionnaire_id == "guardian-signature-part-1":
                            ...
                            for current_response in questionnaire_response.item:
                                ...
                                if current_response.answer[0].valueBoolean:
                                    ...
                                    assent_exceptions.append(SFHIR._exception_description(answer.text))
        ...
        # Link back to participant
        consent_object['ppm_id'] = SFHIR._get_referenced_id(questionnaire_response.as_json(), 'Patient')
    ...
    def flatten_list(bundle, resource_type):
        ...
        resource = SFHIR._get_list(bundle, resource_type)
        if not resource:
    ...
    def flatten_document_references(bundle):
        ...
        references = SFHIR._get_resources(bundle, 'DocumentReference')

        # Flatten them
        flattened = []
        for reference in references:

            flattened.append(SFHIR.flatten_document_reference(reference))
    ...
    def flatten_document_reference(resource):
        ...
        resource = SFHIR._get_or(resource, ['resource'], resource)

        # Pick out properties and build a dict
        reference = {'id': SFHIR._get_or(resource, ['id'])}

        # Get dates
        reference['timestamp'] = SFHIR._get_or(resource, ['indexed'])
        if reference.get('timestamp'):
            reference['date'] = SFHIR._format_date(reference['timestamp'], '%m/%d/%Y')

        # Get data provider
        reference['code'] = SFHIR._get_or(resource, ['type', 'coding', 0, 'code'])
        reference['display'] = SFHIR._get_or(resource, ['type', 'coding', 0, 'display'])

        # Get data properties
        reference['title'] = SFHIR._get_or(resource, ['content', 0, 'attachment', 'title'])
        reference['size'] = SFHIR._get_or(resource, ['content', 0, 'attachment', 'size'])
        reference['hash'] = SFHIR._get_or(resource, ['content', 0, 'attachment', 'hash'])
        reference['url'] = SFHIR._get_or(resource, ['content', 0, 'attachment', 'url'])
        ...
        # Get person
        reference['patient'] = SFHIR._get_or(resource, ['subject', 'reference'])
        if reference.get('patient'):
            reference['ppm_id'] = reference['fhir_id'] = SFHIR._get_referenced_id(resource, 'Patient')

        # Check for data
        reference['data'] = SFHIR._get_or(resource, ['content', 0, 'attachment', 'data'])
    ...
    class Resources:
        ...
        def ppm_research_study(project, title):
            ...
            data = {
                ...
                'id': project,
                'identifier': [{
                    'system': SFHIR.research_study_identifier_system,
                    'value': f'ppm-{project}'
                }],
                'status': 'in-progress',
        ...
        def ppm_research_subject(project, patient_ref, status='candidate', consent=None):
            ...
            data = {
                'resourceType': 'ResearchSubject',
                'identifier': {
                    'system': SFHIR.research_subject_identifier_system,
        ...
        def ppm_device(item, patient_ref, identifier=None, shipped=None, returned=None, status='active'):
            ...
            data = {
                ...
                'type': {
                    'coding': [{
                        'system': SFHIR.device_coding_system,
                        'code': item,
            ...
            if identifier:
                data['identifier'] = [{
                    'system': SFHIR.device_identifier_system,
                    'value': identifier
                }]
        ...
        def communication(patient_ref, identifier, content=None,
            ...
            data = {
                ...
                'identifier': [{
                    'system': SFHIR.ppm_comm_identifier_system,
                    'value': identifier,
                }],
                'sent': sent,
        ...
        def patient(form):
            ...
            patient_data = {
                ...
                'identifier': [
                    {
                        'system': SFHIR.patient_email_identifier_system,
                ...
                'telecom': [
                    {
                        'system': SFHIR.patient_phone_telecom_system,
            ...
            if form.get('contact_email'):
                ...
                patient_data['telecom'].append({
                    'system': SFHIR.patient_email_telecom_system,
            ...
            if form.get('how_did_you_hear_about_us'):
                ...
                patient_data['extension'] = [
                    {
                        "url": SFHIR.referral_extension_url,
            ...
            if form.get('twitter_handle'):
                ...
                patient_data['telecom'].append({
                    'system': SFHIR.patient_twitter_telecom_system,
